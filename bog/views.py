...
def add_comment_to_post(request, pk):
    ...
    if request.method == "POST":
        ...
        form = CommentForm()
    return render(request, 'bog/add_comment_to_post.html', {'form': form})

@login_required
def comment_approve(request, pk):
    comment = get_object_or_404(Comment, pk=pk)
    comment.approve()
    return redirect('bog.views.post_detail', pk=comment.post.pk)
...
def comment_remove(request, pk):
    ...
    comment.delete()
    return redirect('bog.views.post_detail', pk=post_pk)
